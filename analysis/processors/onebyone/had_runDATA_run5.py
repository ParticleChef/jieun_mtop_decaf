...
#                    'RunNumber',
        ...
        for region, cuts in regions.items():
            ...
            if isData:
                ...
                for ij, ijcut in enumerate(cuts):
                    print('consider cut ', ijcut)
                    allcuts_.add(ijcut)
                    temp_cut = selection.all(*allcuts_)
                    if ijcut == 'no_loose_pho':
                        print('====', ijcut, '====')
                        for iii in range(len(temp_cut)):
                            if iii > 20: continue
                            if temp_cut[iii] == False: continue
                            print(iii, temp_cut[iii])
                            print('pho pt', pho.pt[iii])
                            print('pho phi', pho.phi[iii])
                            print('pho eta', pho.eta[iii])
                            print('pho cutBased', pho['cutBased'][iii])
                            print('n clean pho', ak.num(pho_clean)[iii])
                            print('n loose pho', pho_nloose[iii])
                            print('dr_pho_mu: ', pho.metric_table(mu_loose)[iii])
                            print('dr_pho_el: ', pho.metric_table(e_loose)[iii])
                        print('photon before cut: ', pho.pt, ak.sum(ak.num(pho, axis=1)))
                        print('loose photon before cut: ', pho.pt, ak.sum(ak.num(pho_loose, axis=1)))
                        print('after no_loose_pho: number of photons', pho[temp_cut], ak.sum(ak.num(pho[temp_cut],axis=1)))
                        print('after no_loose_pho: number of loose photons', pho_loose[temp_cut], ak.sum(ak.num(pho_loose[temp_cut],axis=1)))
                        print('drGM: ', pho.metric_table(mu_loose)[temp_cut])
                        print('photon after allcuts: ', pho.pt, ak.sum(ak.num(pho[jcut].pt, axis=1)))
                        print('loose photon after allcuts: ', pho_loose[jcut].pt, ak.sum(ak.num(pho_loose[jcut].pt, axis=1)))
                        print('')
                    if ijcut == 'one_loose_mu':
                        print('====', ijcut, '====')
                        print('muon before cut: ', mu.pt, ak.sum(ak.num(mu, axis=1)))
                        print('loose muon before cut: ', mu_loose.pt, ak.sum(ak.num(mu_loose, axis=1)))
                        print('after one_loose_mu: number of mu', mu[temp_cut], ak.sum(ak.num(mu[temp_cut],axis=1)))
                        print('after one_loose_mu: number of loose mu', mu_loose[temp_cut], ak.sum(ak.num(mu_loose[temp_cut],axis=1)))
                        print('muon after allcuts: ', pho.pt, ak.sum(ak.num(mu[jcut].pt, axis=1)))
                        print('loose muon after allcuts: ', mu_loose[jcut].pt, ak.sum(ak.num(mu_loose[jcut].pt, axis=1)))
                        print('')
                    if ijcut == 'one_tight_mu':
                        print('====', ijcut, '====')
                        print('muon before cut: ', mu.pt, ak.sum(ak.num(mu, axis=1)))
                        print('tight muon before cut: ', mu_tight.pt, ak.sum(ak.num(mu_tight, axis=1)))
                        print('after one_tight_mu: number of muons', mu[temp_cut], ak.sum(ak.num(mu[temp_cut],axis=1)))
                        print('after one_tight_mu: number of tight muons', mu_tight[temp_cut], ak.sum(ak.num(mu_tight[temp_cut],axis=1)))
                        print('muon after allcuts: ', pho.pt, ak.sum(ak.num(mu[jcut].pt, axis=1)))
                        print('tight muon after allcuts: ', mu_tight[jcut].pt, ak.sum(ak.num(mu_tight[jcut].pt, axis=1)))
                        print('')
                    if ijcut == 'no_loose_e':
                        print('====', ijcut, '====')
                        print('ele before cut: ', e.pt, ak.sum(ak.num(e, axis=1)))
                        print('tight muon before cut: ', e_loose.pt, ak.sum(ak.num(e_loose, axis=1)))
                        print('after no_loose_e: number of e', e[temp_cut], ak.sum(ak.num(e[temp_cut],axis=1)))
                        print('after no_loose_e: number of tight e', e_tight[temp_cut], ak.sum(ak.num(e_tight[temp_cut],axis=1)))
                        print('e after allcuts: ', e.pt, ak.sum(ak.num(e[jcut].pt, axis=1)))
                        print('tight e after allcuts: ', e_tight[jcut].pt, ak.sum(ak.num(e_tight[jcut].pt, axis=1)))
                        print('')
                kcut = selection.all(*allcuts_)
                print('kcut', kcut, len(kcut))
                selected_pho = pho[kcut]
