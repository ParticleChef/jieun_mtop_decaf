...
#            genIsoAs = gen[gen.isIsoA & (gen.pt > 100)]
            ...
            if ('WJetsToLNu' in dataset) and ('0J' not in dataset):
...
###########################################
        ...
        for region, cuts in regions.items():
            ...
            if not region == 'tmcr': # or  region == 'gcr' or region == 'zecr' or region == 'zmcr':
                print('skip')
                continue
...
#    correctionsBtag = load('data/correction_btag.coffea')
 ...
 #                                          correctionsBtag=correctionsBtag,
    ...
    if options.year  == '2016':
        save(processor_instance, 'data/runSYS_'+options.year+options.vfp+'_Run'+options.run+'.processor')
        print("processor name: runSYS_{}{}_Run{}".format(options.year,options.vfp,options.run))
    else:
        save(processor_instance, 'data/runSYS_'+options.year+'_Run'+options.run+'.processor')
        print("processor name: runSYS_{}_Run{}".format(options.year,options.run))
