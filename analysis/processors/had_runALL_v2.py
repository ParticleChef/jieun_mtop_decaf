    ...
    def __init__(self, year, xsec,corrections,  correctionsUL, ids, common, vfp):
        ...
        self._accumulator = processor.dict_accumulator({
            ...
            'elephi': hist.Hist(
                ...
                hist.Cat('region', 'Region'),
                hist.Cat('systematic', 'Systematic'),
                hist.Bin('elephi', 'Electron $\Phi$', 35,-3.5,3.5)
            ),
            ...
            'phophi': hist.Hist(
                ...
                hist.Cat('region', 'Region'),
                hist.Cat('systematic', 'Systematic'),
                hist.Bin('phophi', 'Photon $\Phi$', 35,-3.5,3.5)
            ),
            ...
            'dphi_fj_recoil': hist.Hist(
                ...
                hist.Cat('region', 'Region'),
                hist.Cat('systematic', 'Systematic'),
                hist.Bin('dphi_fj_recoil', '$\Delta\Phi (leading AK15 jet, recoil)$', 30,0.,3.5)
            ),
            ...
            'dphi_j_met': hist.Hist(
                ...
                hist.Cat('region', 'Region'),
                hist.Cat('systematic', 'Systematic'),
                hist.Bin('dphi_j_met', '$\Delta\Phi (AK4 jet, MET)$', 30,0.,3.5)
            ),
            'partWpt': hist.Hist(
                'Events',
                hist.Cat('dataset', 'dataset'),
...
###########################################
        ...
        #selection.add('dPhi_recoil_j_e', ak.to_numpy(ak.min(abs(corrected_met.delta_phi(mete_j.j)), axis = 1)>0.5))
        #selection.add('dPhi_recoil_j_m', ak.to_numpy(ak.min(abs(corrected_met.delta_phi(metm_j.j)), axis = 1)>0.5))
        #selection.add('dPhi_recoil_j_g', ak.to_numpy(ak.min(abs(corrected_met.delta_phi(metg_j.j)), axis = 1)>0.5))
        ...
        for region, cuts in regions.items():
            ...
            if region == 'zecr' or region == 'zmcr' or region == 'sr':
                continue
...
#                    if os.path.exists("{0}_{1}_{2}.root".format(self._year, region, dataset)) == False:
#                        print("Making {0}_{1}_{2}.root".format(self._year, region, dataset))
#                        f = uproot.recreate("{0}_{1}_{2}.root".format(self._year, region, dataset))
#                        f["Events"] = {
#                            "corMET": ak.Array(corrected_met.pt),
#                           # "corMET_cut": ak.Array(corrected_met[cut].pt),
#                            "corMETphi": ak.Array(corrected_met.phi),
#                            "MET": ak.Array(events.MET.pt),
#                            "METphi": ak.Array(events.MET.phi),
#                            "l1pt": ak.Array(events.Electron.pt),
#                            "l2pt": ak.Array(events.Electron.pt),
#                            "j1pt": ak.Array(events.Jet.pt),
#                            "fjpt": ak.Array(events.AK15PFPuppi.Jet_pt),
#                            "recoil": ak.Array(u[region].pt),
#                            "recoilphi": ak.Array(u[region].phi),
#                            "genWpt_cut": ak.Array(gen[gen.hasFlags(['fromHardProcess', 'isFirstCopy', 'isPrompt']) & (abs(gen.pdgId) == 24) & cut].pt),
#                            "genWpt": ak.Array(gen[gen.hasFlags(['fromHardProcess', 'isFirstCopy', 'isPrompt']) & (abs(gen.pdgId) == 24)].pt),
#                        }
#                        f.close()
#                    else:
#                        print("Appending {0}_{1}_{2}.root".format(self._year, region, dataset))
#            
#                    f = uproot.open("{0}_{1}_{2}.root".format(self._year, region, dataset))
#                    rootOut = f["Events"].arrays()
#                    print(rootOut)
#                    f.close()
#                    newOut = {
#                        "corMET": ak.concatenate([rootOut["corMET"],ak.Array(corrected_met.pt)]),
#                       # "corMET_cut": ak.concatenate([rootOut["corMET"],ak.Array(corrected_met[cut].pt)]),
#                        "corMETphi": ak.concatenate([rootOut["corMETphi"],ak.Array(corrected_met.phi)]),
#                        "MET": ak.concatenate([rootOut["MET"],ak.Array(events.MET.pt)]),
#                        "METphi": ak.concatenate([rootOut["METphi"],ak.Array(events.MET.phi)]),
#                        "l1pt": ak.concatenate([rootOut["l1pt"],ak.Array(events.Electron.pt)]),
#                        "l2pt": ak.concatenate([rootOut["l2pt"],ak.Array(events.Electron.pt)]),
#                        "j1pt": ak.concatenate([rootOut["j1pt"],ak.Array(events.Jet.pt)]),
#                        "fjpt": ak.concatenate([rootOut["fjpt"],ak.Array(events.AK15PFPuppi.Jet_pt)]),
#                        "recoil": ak.concatenate([rootOut["recoil"],ak.Array(u[region].pt)]),
#                        "recoilphi": ak.concatenate([rootOut["recoilphi"],ak.Array(u[region].phi)]),
#                        "genWpt_cut": ak.concatenate([rootOut["genWpt"],ak.Array(gen[gen.hasFlags(['fromHardProcess', 'isFirstCopy', 'isPrompt']) & ((abs(gen.pdgId) == 24)) & cut].pt)]),
#                        "genWpt": ak.concatenate([rootOut["genWpt"],ak.Array(gen[gen.hasFlags(['fromHardProcess', 'isFirstCopy', 'isPrompt']) & ((abs(gen.pdgId) == 24))].pt)]),
#                    }
#                    f = uproot.recreate("{0}_{1}_{2}.root".format(self._year, region, dataset))
#                    f["Events"] = newOut
#                    f.close()
    ...
    def postprocess(self, accumulator):
        ...
        for d in accumulator['sumw'].identifiers('dataset'):
            ...
            if self._xsec[dataset] != -1:
                if re.search("^WJetsToLNu_[012]J_", dataset):
                    k_fac_fewz = 61526.7/65582
                    scale[d.name] = (self._lumi*self._xsec[dataset]*k_fac_fewz)
...
#    correctionsBtag = load('data/correction_btag.coffea')
 ...
 #                                          correctionsBtag=correctionsBtag,
    ...
    if options.year  == '2016':
        save(processor_instance, 'data/manyplot_'+options.year+options.vfp+'_V'+options.run+'.processor')
        print("processor name: manyplot_{}{}_V{}".format(options.year,options.vfp,options.run))
    else:
        save(processor_instance, 'data/manyplot_'+options.year+'_V'+options.run+'.processor')
        print("processor name: manyplot_{}_V{}".format(options.year,options.run))
