...
#         leading_dimu = dimu[dimu.pt.argmax()]
        ...
        u = { # recoil
            'sr': met + np.zeros_like(leading_e),
            'wmcr': metm,
            'tmcr': metm,
            'wecr': mete,
            'tecr': mete,
...
#             selection.add("exclude_low_WpT_JetHT", np.full(event_size, True))
        ...
        selection.add('isoneM', ak.to_numpy((e_nloose == 0) & (mu_ntight == 1) & ( mu_nloose == 1) & (tau_nloose == 0) & (pho_nloose == 0)))
...
#            selection.add(sel_name, ak.to_numpy(ak.sum(select,axis=1)>0))
        ...
        regions = {
            'sr': [ 'fatjet', 'noHEMj', 'iszeroL', 'noHEMmet', 'met_filters', 'met_triggers', 'noextrab'],
            'wmcr': ['isoneM', 'fatjet', 'noHEMj', 'met_filters', 'dPhi_recoil_j_m', 'dPhi_recoil_fj_m', 'met_triggers', 'noextrab'],
            'tmcr': ['isoneM', 'fatjet', 'noHEMj', 'met_filters', 'dPhi_recoil_j_m', 'dPhi_recoil_fj_m', 'met_triggers', 'extrab'],
...
#               '\n', 'e_ntight->', e_ntight.sum())
        for region, cuts in regions.items():
            ...
            selection.add('recoil_'+region, ak.to_numpy(ak.sum(u[region].pt, axis = 1) > 250))
            ...
            regions[region].insert(1, 'calo_'+region)
