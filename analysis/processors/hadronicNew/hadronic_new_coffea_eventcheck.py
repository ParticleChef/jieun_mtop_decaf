...
# not updated JUNC
        ...
        self._accumulator = processor.dict_accumulator({
            ...
            'nevents': hist.Hist(
                'sumevents',
                hist.Cat('dataset', 'Dataset'),
                hist.Bin('nevents', 'number of events', [0.])),
...
#www#                                       isolation_name+'Up', isolation_name+'Down',
                    ...
                    if not isFilled:
                        ...
                        #hout['nevents'].fill(dataset=dataset, nevents=1, weight=np.ones_like(weights.weight()))
...
#                scale[d.name] = 1
        ...
        for histname, h in accumulator.items():
            if histname == 'sumw':
                print(h.values())
                print(h.integrate('dataset').values()[()])
                print(h.integrate('dataset').sum().values()[()])
                continue
            if histname == 'nevents':
                continue
