...
# not updated JUNC
        ...
        self._accumulator = processor.dict_accumulator({
            ...
            'eT_miss': hist.Hist(
                ...
                hist.Cat('dataset', 'Dataset'),
                hist.Cat('region', 'Region'),
                hist.Bin('eT_miss', '$E^T_{miss}$[GeV]', 20, 0, 1000)),
            'recoil_sr': hist.Hist(
                'Events',
                hist.Cat('dataset', 'Dataset'),
                hist.Cat('region', 'Region'),
...
#         leading_dimu = dimu[dimu.pt.argmax()]
        ...
        print('met: ', ak.type(met))
        print('pho: ', ak.type(leading_pho))
        print('ele: ', ak.type(leading_e))
        #mete = epho + met
        #metm = mpho + met
        mete = leading_e + met
        metm = leading_mu + met
        print('mete ', ak.type(mete))
        ...
        #print('met.pt: ', met.pt)
        #print('mete: ', mete)
        #print('e.pt + met.pt: ', leading_e.pt + met.pt)
        #print('one like + met: ', met + np.ones_like(leading_e))
        #print('zero like + met: ', met + np.zeros_like(leading_e))
        #print('zero like + met pt: ', (met + np.zeros_like(leading_e)).pt)
        #print('zero like met + met pt: ', (met + np.zeros_like(met)).pt)


        #print('met.pt: ', met.pt)
        #sr_recoil = met+np.zeros_like(leading_e)
        #print('wr_recoil: ', sr_recoil.pt)

        #print('mete_j: ', mete_j)
        #print('dphi(recoil,j): ', mete_j.mete.delta_phi(mete_j.j)) 
...
#             selection.add("exclude_low_WpT_JetHT", np.full(event_size, True))
        ...
        print(ak.to_numpy(ak.sum(leading_fj.pt, axis=1)>160))
...
#            selection.add(sel_name, ak.to_numpy(ak.sum(select,axis=1)>0))
        ...
        selection.add('dPhi_recoil_j_e', ak.to_numpy(ak.sum(abs(mete_j.mete.delta_phi(mete_j.j))<0.5, axis=1)==0))
        print('mete_j_dphi', abs(mete_j.mete.delta_phi(mete_j.j))<0.5))
        selection.add('dPhi_recoil_j_m', ak.to_numpy(ak.sum(abs(metm_j.metm.delta_phi(metm_j.j))<0.5, axis=1)==0))
        ...
        regions = {
            ...
            'wmcr': ['met_filters', 'met_triggers', 'fatjet_good_leading', 'isoneM', 'noextrab', 'dPhi_recoil_j_m', 'dPhi_recoil_fj_m', 'noHEMj'],
            'tmcr': ['met_filters', 'met_triggers','fatjet_good_leading', 'isoneM', 'extrab', 'dPhi_recoil_j_m', 'dPhi_recoil_fj_m', 'noHEMj'],
            ##for seperate isoneM##'tmcr': ['met_filters', 'met_triggers','fatjet_leading', 'fatjet_good_leading', 'isoneM_noE','isoneM_onetightMu', 'isoneM_onelooseMu','isoneM_notau','isoneM_nopho', 'extrab', 'dPhi_recoil_j_m', 'dPhi_recoil_fj_m', 'noHEMj'],
            'wecr': ['met_filters', 'fatjet_good_leading', 'isoneE', 'noextrab', 'dPhi_recoil_j_e', 'dPhi_recoil_fj_e', 'met100', 'single_electron_triggers', 'noHEMj'],
            'tecr': ['met_filters', 'fatjet_good_leading', 'isoneE', 'extrab', 'dPhi_recoil_j_e', 'dPhi_recoil_fj_e', 'met100', 'single_electron_triggers', 'noHEMj'],
            'zmcr': ['met_filters', 'met_triggers', 'fatjet_good_leading', 'isoneM', 'noHEMj'],
            'zecr': ['met_filters', 'fatjet_good_leading', 'isoneE', 'single_electron_triggers', 'noHEMj'],
            'gcr': ['met_filters', 'isoneE', 'fatjet_good_leading', 'noHEMj']
        }
...
if __name__ == '__main__':
    ...
    save(processor_instance, 'data/UL_had_ttag_cutflow_'+options.year+'_v5run.processor')
    print("processor name: UL_had_ttag_cutflow_{}_v5".format(options.year))
