...
#        Jetevaluator = self._corrections['Jetevaluator'] ###
        ...
        #mu['isloose'] = isLooseMuon(mu.pt, mu.eta, mu.pfRelIso04_all, mu.pfIsoId, self._year)
        #mu['isloose'] = isLooseMuon(mu.pt, mu.eta, mu.pfRelIso04_all, mu.looseId, self._year, events.Muon.isPFcand, events.Muon.isGlobal, events.Muon.isTracker)
        ...
        #mu['istight'] = isTightMuon(mu.pt, mu.eta, mu.pfRelIso04_all, mu.tightId, self._year, events.Muon.isPFcand, events.Muon.isGlobal, events.Muon.isTracker)#, mu.pfIsoId)
...
#         ak.all(tau.metric_table(mu_loose) > 0.4, axis=-1)
        ...
        fj_pt_mask = fj.pt > 160
        fj_eta_mask = abs(fj.eta) < 2.4
        fj_id_mask = (fj.Jet_jetId&2) == 2

        fj_pt_check = fj[fj_pt_mask]
        fj_eta_check = fj[fj_eta_mask]
        fj_id_check = fj[fj_id_mask]

        fj_isclean_mask = (fjMuMask & fjEleMask & fjPhoMask)
        fj_isgood_mask = isGoodFatJet(fj.pt, fj.eta, fj.Jet_jetId)
        #fj_isgood_mask = (fj.pt > 160)
        ...
        #leading_fj_good = fj[fj_isclean_mask][:,:1] # test for cutflow isGoodFatJet seperately
...
#         j_dflvL = j_clean[j_clean.isdflvL.astype(np.bool)]
        ...
        #dibjet = j_good_clean_dflvB[:,0] + j_good_clean_dflvB[:,1]
        #print('dibjet: ', dibjet)
        #print('dibjet pt: ', dibjet.pt)
...
#         leading_dimu = dimu[dimu.pt.argmax()]
        ...
        print('corrected_met: ', corrected_met.T, ak.type(corrected_met.T))
        print('ele: ', e.T, ak.type(e.T))
        print('leading ele: ', leading_e.T, ak.type(leading_e.T))
        print('leading mu: ', leading_mu.T, ak.type(leading_mu.T))
        recoil_e = corrected_met + leading_e
        recoil_m = corrected_met + leading_mu
        print('test_recoil: ', recoil_e)
        print('test_recoil pt: ', recoil_e.pt)
        test_leading_recoil_e = corrected_met + leading_e
        test_leading_recoil_m = corrected_met + leading_mu
        #print('leading test_recoil_e: ', test_leading_recoil_e)
        #print('leading test_recoil_e pt: ', test_leading_recoil_e.pt)
        #print('leading test_recoil_m: ', test_leading_recoil_m)
        #print('leading test_recoil_m pt: ', test_leading_recoil_m.pt)
        
        #mete = leading_e + met
        #metm = leading_mu + met
        mete = corrected_met + leading_e
        metm = corrected_met + leading_mu
...
#             selection.add("exclude_low_WpT_JetHT", np.full(event_size, True))
        ...
        selection.add('fj_pt_check', ak.to_numpy(ak.num(fj_pt_check)>0))
        selection.add('fj_eta_check', ak.to_numpy(ak.num(fj_eta_check)>0))
        selection.add('fj_id_check', ak.to_numpy(ak.num(fj_id_check)>0))
        ...
        selection.add('fatjetpt', ak.to_numpy(ak.sum(leading_fj.pt, axis=1)>160))
        ...
        regions = {
            ...
            #'tmcr': ['met_filters', 'met_triggers', 'isoneM', 'extrab', 'dPhi_recoil_j_m', 'dPhi_recoil_fj_m', 'noHEMj'],
            ##'tmcr': ['met_filters', 'met_triggers','fj_pt_check', 'fj_eta_check','fj_id_check','fj_clean', 'isoneM', 'extrab', 'dPhi_recoil_j_m', 'dPhi_recoil_fj_m', 'noHEMj'],
            'tmcr': ['met_filters', 'met_triggers','isoneM_nopho','isoneM_onelooseMu','isoneM_onetightMu','isoneM_noE', 'fj_ngood_clean', 'dPhi_recoil_fj_m','dPhi_recoil_j_m', 'noHEMj'],
...
if __name__ == '__main__':
    ...
    with open('metadata/KIT_UL_2018_vMET.json') as fin:
    ...
    save(processor_instance, 'data/UL_had_ttag_cutflow_'+options.year+'_New_v2.processor')
    print("processor name: UL_had_ttag_cutflow_{}_New_v2".format(options.year))
