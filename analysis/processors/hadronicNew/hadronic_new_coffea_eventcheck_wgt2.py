...
#                'Ele115_CaloIdVT_GsfTrkIdT',
            ...
            '2018': [
#                'Ele50_CaloIdVT_GsfTrkIdT_PFJet165', # Add 2022 Dec8
                'Ele32_WPTight_Gsf',
#                'Ele115_CaloIdVT_GsfTrkIdT',
                'Photon200'
            ]
...
#                     nnlo_nlo[systematic] = get_nnlo_nlo_weight['a'][systematic](genIsoAs.pt.max())*ak.values_astype((ak.num(genIsoAs,axis=1) > 0), np.int) + ak.values_astype(~(ak.num(genIsoAs, axis=1) > 0), np.int)
            ...
            #e1sf = get_ele_trig_weight(ak.sum(ele_pairs.one_e.eta)+ak.sum(ele_pairs.one_e.deltaEtaSC),ak.sum(ele_pairs.one_e.pt))*ak.values_astype(ak.sum(ele_pairs.one_e.pt) > 40, np.int)
            #e2sf = get_ele_trig_weight(ak.sum(ele_pairs.two_e.eta)+ak.sum(ele_pairs.two_e.deltaEtaSC),ak.sum(ele_pairs.two_e.pt))*ak.values_astype(ak.sum(ele_pairs.two_e.pt) > 40, np.int)
...
###########################################
        ...
        selection.add('iszeroTau',ak.to_numpy((tau_nloose == 0)))
        selection.add('iszeroG',ak.to_numpy((pho_nloose == 0)))
        ...
        regions = {
            'sr': [ 'met_filters', 'met_triggers','iszeroL',
                    'fatjet_leading', 'fatjet_good_leading', 
                    'noextrab', 'noHEMj', 'noHEMmet'],

            'wmcr': [ 'met_filters','met_triggers',
                    #'isoneM', 'iszeroG',
                    'no_loose_pho','one_loose_mu','one_tight_mu','no_loose_e',
                    'fj_ngood_clean', 'recoil_wmcr','dPhi_recoil_j_m', 'dPhi_recoil_fj_m', 
                    'noHEMj', 'noextrab'],

            'wecr': ['met_filters','single_electron_triggers','isoneE', 'iszeroG', 
                    'fj_ngood_clean', 'recoil_wecr', 'dPhi_recoil_j_e', 'dPhi_recoil_fj_e', 'met100',
                    'noextrab','noHEMj'],

            'tmcr': ['met_filters', 'met_triggers',
                    #'isoneM', 'iszeroG',
                    'no_loose_pho','one_loose_mu','one_tight_mu','no_loose_e',
                    'fj_ngood_clean','recoil_tmcr', 'dPhi_recoil_j_m','dPhi_recoil_fj_m', 
                    'noHEMj'],

            'tecr': ['met_filters', 'single_electron_triggers','isoneE', 'iszeroG',
                    'fj_ngood_clean','recoil_tecr','dPhi_recoil_fj_e', 'dPhi_recoil_j_e', 'met100',
                    'noHEMj','extrab'],

            'zmcr': [ 'met_filters', 'met_triggers','istwoM', 'iszeroG',
                    'fj_ngood_clean', 'met120', 
                    'noHEMj'],

            'zecr': [ 'met_filters', 'met_triggers','istwoE', 'iszeroG',
                    'fj_ngood_clean', 'met120',
                    'noHEMj'],

            'gcr': [ 'met_filters','isoneG','iszeroG', 
                    'fj_ngood_clean', 
                    'noHEMj']
        }
...
################################################################
                ...
                weights = Weights(len(events))
                #www#if 'L1PreFiringWeight' in events.fields:
                #www#    weights.add('prefiring', events.L1PreFiringWeight.Nom)
...
#         time.sleep(0.5)
    ...
    def postprocess(self, accumulator):
        ...
        for d in accumulator['sumw'].identifiers('dataset'):
            nevents = accumulator['sumw'].integrate('dataset', d).values(overflow='all')[()][1]
            print('sumw', nevents)
...
if __name__ == '__main__':
    ...
    #with open('metadata/KIT_UL_2018_v5.json') as fin:
    ...
    save(processor_instance, 'data/UL_had_'+options.year+'_run_cutflowcheck.processor')
    print("processor name: UL_had_{}_run_v1".format(options.year))
