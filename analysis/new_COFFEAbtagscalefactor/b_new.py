...
class BTagScaleFactor:
    ...
    FLAV_B, FLAV_C, FLAV_UDSG = numpy.array([5,4,0])
    _formulaLock = Lock()
    _formulaCache = {}
    _btvflavor = numpy.array([0, 4, 5, 6])
    ...
    def readcsv(cls, filename):
        ...
        df['OperatingPoint'].mask(df['OperatingPoint'] == 'L', 0, inplace=True)
        df['OperatingPoint'].mask(df['OperatingPoint'] == 'M', 1, inplace=True)
        df['OperatingPoint'].mask(df['OperatingPoint'] == 'T', 2, inplace=True)
