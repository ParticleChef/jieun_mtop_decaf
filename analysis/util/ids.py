...
def isTightElectron(pt,eta,dxy,dz,tight_id,year):
    ...
    if year=='2016':
        ...
        mask = ((pt>40)&(abs(eta)<1.4442)&(abs(dxy)<0.05)&(abs(dz)<0.1)&(tight_id==4)) | ((pt>40)&(abs(eta)>1.5660)&(abs(eta)<2.4)&(abs(dxy)<0.1)&(abs(dz)<0.2)&(tight_id==4)) #       Trigger: HLT_Ele32_WPTight_Gsf_v
    return mask
...
###### Include eta conditions
    #Tight photon use medium ID, as in monojet
    mask = ~(pt==np.nan)#just a complicated way to initialize a jagged array with the needed shape to True
    if year=='2016':
        mask = (pt>200)&~((abs(eta)>1.4442)&(abs(eta)<1.5660))&(abs(eta)<1.479)&(tight_id>=2) # Trigger threshold is at 175
    elif year=='2017':
        mask = (pt>230)&~((abs(eta)>1.4442)&(abs(eta)<1.5660))&(abs(eta)<1.479)&((tight_id&2)==2) # Trigger threshold is at 200
    elif year=='2018':
        mask = (pt>230)&~((abs(eta)>1.4442)&(abs(eta)<1.5660))&(abs(eta)<1.479)&((tight_id&2)==2) # Trigger threshold is at 200
    return mask

def isGoodFatJet(pt, eta, jet_id, chf, nhf):
    mask = (pt > 160) & (abs(eta)<2.4) & ((jet_id&4)==4) & (chf>0.1) & (nhf<0.8)
    return mask

def isGoodJet(pt, eta, jet_id, pu_id, nhf, chf, year):
    #https://twiki.cern.ch/twiki/bin/view/CMS/PileupJetID, using loose wp
    #jetId==6 means: pass tight and tightLepVeto ID. https://twiki.cern.ch/twiki/bin/viewauth/CMS/JetID
    if year=='2016':
        mask = (pt>=30) & (abs(eta)<=2.4) & (jet_id>=4) #& (chf>0.1) & (nhf<0.8) #& (chf<0.8) & (nhf>0.1) 
        mask = ((pt>=50)&mask) | ((pt<50)&mask&((pu_id & (0 << 1))==1)) 
    else:
        mask = (pt>=30) & (abs(eta)<=2.4) & (jet_id>=4) #& (chf>0.1) & (nhf<0.8) #& (chf<0.8) & (nhf>0.1)
        mask = ((pt>=50)&mask) | ((pt<50)&mask&((pu_id & (1 << 2)) ==4)) 
        # puId&4)==4 jet pass atleast the loose pu_id, https://twiki.cern.ch/twiki/bin/viewauth/CMS/PileupJetID#Recommendations_for_13_TeV_Ultra
#matteo
